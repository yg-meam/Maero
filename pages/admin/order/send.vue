...
export default {
            ...
            subMenu: [
              {
                ...
                label: "발주전",
                action: () => {
                  console.log(this.orderList[rowKey]);
      ...
      columns: [
        ...
        },
        {
          ...
          formatter(item) {
            ...
            // }
            if (item.value) {
              var finded = _.find(orderType, (orderType) => {
                return orderType.value == item.value;
              });
              if (finded) {
                return finded.text;
              }
        ...
        },
        {
          ...
          formatter: (item) => {
            if (item.value) {
              var finded = _.find(this.packageList, (box) => {
        ...
        },
        {
          ...
          sortable: true,
          formatter(data) {
            if (data.row.hscodes && data.row.hscodes[0]) {
              return data.row.hscodes[0].product;
            }
          },
        },
        {
          ...
          formatter: (item) => {
            if (item.value) {
              var finded = _.find(this.hscodeList, (hscode) => {
                return hscode.value == item.value;
              });
              if (finded) {
                return finded.text;
              }
  ...
  methods: {
    ...
    getHscodeList() {
      this.$axios.post("/admin/hscode/list").then((result) => {
        if (result.data.res) {
          var hscodeColumn = _.find(this.columns, (item) => {
            return item.name == "hscode";
          });
          var index = _.indexOf(this.columns, hscodeColumn);

          this.hscodeList = _.map(result.data.list, (item) => {
            return {
              text: item.kind,
              value: item.hscode,
            };
          });
          this.columns[index].editor.options.listItems = this.hscodeList;

          // var fieldName = _.find(this.columns, (item) => {
          //   return item.name == "partnerShop";
          // });
          // var shopIndex = _.indexOf(this.columns, fieldName);

          // this.shopList = _.map(result.data.marketList, (item) => {
          //   return {
          //     text: item.market,
          //     value: item.marketUrl,
          //   };
          // });
          // this.columns[shopIndex].editor.options.listItems = this.shopList;

          var packageColumn = _.find(this.columns, (item) => {
            return item.name == "PackageId";
          });
          var packageIndex = _.indexOf(this.columns, packageColumn);

          this.packageList = _.map(result.data.packageList, (item) => {
            return {
              text: item.no,
              value: item.no,
            };
          });
          this.columns[packageIndex].editor.options.listItems =
            this.packageList;

          this.isHscodeInit = true;
        ...
        .then((result) => {
          if (result.data.res) {
            ...
            _.each(event.changes, (change) => {
              ...
              changeFeild[change.columnName] = change.value;
              changeFeildBox.push(changeFeild);
            });
          }
        });
    },
    ...
    changeColor(item, rowKey) {
      this.$refs.grid1.invoke("setRow", rowKey, this.orderList[rowKey]);
      if (item.columnName == "colorSize") {
    ...
    setRow(index, changeItem) {
      ...
      if (changeItem.isBuyerNameChange == true) {
        this.$refs.grid1.invoke(
          "addCellClassName",
          index,
          "buyerName",
      ...
      if (changeItem.isZipcodeChange == true) {
        this.$refs.grid1.invoke("addCellClassName", index, "zipcode", "yellow");
      }
      if (changeItem.isAddressChange == true) {
        this.$refs.grid1.invoke("addCellClassName", index, "address", "yellow");
      }
      if (changeItem.isBuyerPhoneChange == true) {
        this.$refs.grid1.invoke(
          "addCellClassName",
          index,
          "buyerPhone",
